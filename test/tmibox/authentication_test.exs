defmodule Tmibox.AuthenticationTest do
  use Tmibox.DataCase

  alias Tmibox.Authentication

  import Tmibox.AuthenticationFixtures
  alias Tmibox.Authentication.{User, UserToken}
  ...
  describe "apply_user_email/3" do
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Authentication.apply_user_email(user, valid_user_password(), %{email: email})
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Authentication.deliver_update_email_instructions(user, "current@example.com", url)
        end)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      token =
        extract_user_token(fn url ->
          Authentication.deliver_update_email_instructions(%{user | email: email}, user.email, url)
        end)
